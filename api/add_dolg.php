...
<form method='post' enctype='multipart/form-data' action='add_dolg.php'>
...
";
	...
	$sql = "DELETE FROM dolg WHERE id>=0";
	mysqli_query($conn,$sql);
	mysqli_query($conn,'SET autocommit=0;');
mysqli_query($conn,'START TRANSACTION;');
	for ($s = 0; $s<count($response1); $s++){
	$sqls="INSERT INTO `dolg` (`id`, `naim`, `nan`, `id_dog`, `kfs`, `stat`, `data`, `nom`, `d_start`, `d_end`, `summa`) VALUES (NULL, '".$response1[$s][0]."', '".$response1[$s][1]."', '".$response1[$s][2]."', '".$response1[$s][3]."', '".$response1[$s][4]."', '".$response1[$s][5]."', '".$response1[$s][6]."', '".$response1[$s][7]."', '".$response1[$s][8]."', '".$response1[$s][9]."');";	
